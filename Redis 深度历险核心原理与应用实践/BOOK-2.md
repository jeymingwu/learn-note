...
+ 快照原理：
    ...
    + Redis 快照持久化机制：使用操作系统的**多线程“写时复制” COW（Copy on write）实现**；
+ fork 多线程
...
+ fsync：
    ...
    + Redis 三种刷回策略：
        + everysec 每秒刷回：每隔一秒刷回；（数据安全性和性能之间的折中）
        + always 每个指令都刷回一次：操作耗时，磁盘 IO 耗时；
        + no 由操作系统配置：不安全，若机器突然宕机，AOF 日志内容可能没有刷回磁盘中；
    + 配置：
        + appendonly yes|no #配置是否开启 AOF 持久化功能，默认不开启状态；
        + appendfsync always|everysec|no #AOF 写数据策略
    + 若 fsync 过程中发生 bgsave 快照存储，先执行快照存储，fsync 被推迟执行；
+ 运维：
